...
export default function Edit({
  ...
  const { data, setData, put, errors } = useForm({
    course_section_id: classSchedule.course_section_id,
    days: '',
    time: '',
    room: '',
  });
  ...
  useEffect(() => {
    const dayArray = classSchedule.days.split('/');
    const timeArray = classSchedule.time.split('/');
    const roomArray = classSchedule.room.split('/');

    const parsed = dayArray.map((day, i) => {
      const [start, end] = (timeArray[i] ?? '').split('-');
      const room = roomArray[i] ?? '';
      const isOnline = room.toLowerCase() === 'online';
      return { day, start, end, room: isOnline ? 'ONLINE' : room, isOnline };
    });
  ...
  function addScheduleBlock() {
    setSchedules([...schedules, { day: '', start: '', end: '', room: '', isOnline: false }]);
  }
  ...
  function submit(e: React.FormEvent) {
    ...
    schedules.forEach((s, i) => {
      ...
      if (!s.start || !s.end) errors.push(`Schedule ${i + 1}: Start and end time are required.`);
      if (!s.isOnline && !s.room.trim()) errors.push(`Schedule ${i + 1}: Room is required for in-person classes.`);
    });
          ...
          <form onSubmit={submit} className="space-y-6 mt-4 max-w-md mx-auto font-sans">
            ...
            <div className="grid gap-2">
              ...
              <AsyncSelect
                ...
                defaultValue={(() => {
                  const match = courseSections.find(cs => cs.id === classSchedule.course_section_id);
                  return match
                    ? {
                      label: `${match.course.title} - ${match.section.code}`,
                      value: match.id,
                    }
                    : null;
                })()}
              ...
              {schedules.map((s, i) => (
                <div key={i} className="grid gap-2 border-b pb-4">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-heading text-[#102d4e]">Schedule {i + 1}</label>
                    <button
                      type="button"
